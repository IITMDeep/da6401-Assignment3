...
def train(model, num_epochs, criterion, optimizer, train_batch_x, train_batch_y, val_batch_x, val_batch_y, df_val, input_char_to_int, output_char_to_int, output_int_to_char, beam_width, length_penalty, cell_type, max_length, wandb_log):
    ...
    for epoch in range(num_epochs):
        ...
        # Total word predict correct over validation
        beam_val_pred = 0
        beam_val = 0
...
## Download HINDI FONT ONLY
def download_hindi_font(font_path, dataset_name='rsrishav/wordcloud-hindi-font', file_name='Nirmala.ttf'):
    """
    Download the Hindi font file if it's missing.

    Args:
        font_path (str): Path to the font file.
        dataset_name (str): Kaggle dataset name containing the font file.
    ...
    if not os.path.exists(font_path):
        # Define the command to download the font file using Kaggle API
        command = f'kaggle datasets download -d {dataset_name} -f {file_name} -p {os.path.dirname(font_path)}'

        # Execute the command to download the font file
        subprocess.run(command, shell=True)
        # Unzip the downloaded file
        zip_file_path = os.path.join(os.path.dirname(font_path), f'{file_name}.zip')
        with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
            zip_ref.extractall(os.path.dirname(font_path))
...
def plot_attention_grid(sentences, translations, attentions, figsize=(15, 15)):
    ...
    font_path = 'wordcloud_hindi_font/Nirmala.ttf'
    download_hindi_font(font_path)

    # Plot each attention matrix in a grid
    for i in range(10):
        ...
        ax.set_xticklabels(sentence, size=8)
        ax.set_yticks(np.arange(len(translation)))

        # Set Hindi font for y-axis tick labels
        hindi_font = FontProperties(fname=font_path) # download this font from kaggle
...
if __name__ == "__main__":
    ...
    parser.add_argument('-hmap', '--heatmap_plot', help='Flag to enable heatmap plot', type=int, default=0, choices=[0, 1])
    ...
    train_inputs, train_outputs = get_tensor_object(df_train, max_length, max_length, input_char_to_int,
                                                    output_char_to_int)
    val_inputs, val_outputs = get_tensor_object(df_val, max_length, max_length, input_char_to_int, output_char_to_int)
    test_inputs, test_outputs = get_tensor_object(df_test, max_length, max_length, input_char_to_int,
                                                  output_char_to_int)
    ...
    # Plot Heat Map if backend support only
    if config.heatmap_plot == 1:
        ...
        for i in range(10):
            ...
            predicted_output, attention = predict(model, input_seq, max_length, input_char_to_int, output_char_to_int,
                                                  output_int_to_char)
            attention = attention[:, :, :(len(input_seq))]
            #     plt.matshow(attention.numpy())
            #     plt.show()
            #     # Ensure beam_search function is defined and imported correctly
            # predicted_output_1, attention = beam_search(model, input_seq, input_char_to_int, output_char_to_int, output_int_to_char, 5, 0.1, "RNN")

            print(f"\n\nInput Sequence {i + 1}: {input_seq}")
            print(f"Output Sequence {i + 1}: {output_seq}")
            print(f"Predicted Sequence {i + 1}: {predicted_output}")
